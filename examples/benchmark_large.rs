...
use inspire_pir::pir::{extract, query_seeded, respond, setup};
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for (name, num_entries) in test_sizes {
        ...
        println!(
            "Database: {} ({} entries, {:.1} MB)",
            name, num_entries, db_size_mb
        );
        ...
        for &idx in &test_indices {
            ...
            let expected = &database
                [target_index as usize * entry_size..(target_index as usize + 1) * entry_size];
            assert_eq!(result, expected, "PIR result mismatch at index {}!", idx);
        }

        let num_queries = test_indices.len() as f64;
        println!(
            "  Avg query gen time:   {:>8.2} ms (seeded)",
            total_query_time.as_secs_f64() * 1000.0 / num_queries
        );
        println!(
            "  Avg respond time:     {:>8.2} ms (expand+respond)",
            total_respond_time.as_secs_f64() * 1000.0 / num_queries
        );
        println!(
            "  Avg extract time:     {:>8.2} ms",
            total_extract_time.as_secs_f64() * 1000.0 / num_queries
        );
