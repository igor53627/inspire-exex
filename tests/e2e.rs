...
use lane_builder::{test_params, HotLaneBuilder, TwoLaneSetup};
...
#[test]
fn test_config_e2e() {
    let config = TwoLaneConfig::from_base_dir("/data/pir").with_entries(1_000_000, 2_700_000_000);
...
fn test_query_size_estimation() {
    ...
    let improvement =
        Lane::Cold.expected_query_size() as f64 / Lane::Hot.expected_query_size() as f64;
...
fn test_privacy_contracts_in_hot_lane() {
    ...
    let privacy_contracts: Vec<_> = manifest
        .contracts
        .iter()
    ...
    assert!(
        !privacy_contracts.is_empty(),
        "Privacy contracts should be in hot lane"
    );
}
...
fn test_pir_database_setup() {
    ...
    let loaded_config =
        TwoLaneConfig::load(&temp_dir.join("config.json")).expect("Config should load");
...
fn test_pir_query_response_cycle() {
    ...
    use inspire_pir::{extract, query, respond};
    ...
    let server_response =
        respond(&result.hot_crs, &result.hot_db, &client_query).expect("Respond should succeed");

    // Client decrypts response
    let retrieved = extract(&result.hot_crs, &client_state, &server_response, entry_size)
    ...
    assert_eq!(
        &retrieved[..],
        expected,
        "Retrieved entry should match original"
    );
